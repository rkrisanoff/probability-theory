...
selection = [-0.34,-0.73,0.69,0.55,1.11,0.62,0.93,0.42,1.00,-0.48,0.92,0.51,-1.05,-1.41,1.04,-1.03,1.55,-0.17,0.92,0.17]
n = len(selection)
...
# %% compute statistic characteristics

Expected_value = sum([value*(count/n) for value,count in discret_statictic_series.items()])

Ranking = variation_series[LAST]-variation_series[FIRST]
    
D = sum([count*(value-Expected_value)**2 for value,count in discret_statictic_series.items()])/n
σ = D ** (1/2)
...
intervals_weight_normalized_based = [round3(sum(intervals_weight_normalized[:i+1])) for i in range(len(intervals_weight_normalized))]

# %% format output
distribution_law = PrettyTable(["intervals"]+[("[{}, {})".format(round3(i-h/2),round3(i+h/2))) for i in intervals[:4]])  
